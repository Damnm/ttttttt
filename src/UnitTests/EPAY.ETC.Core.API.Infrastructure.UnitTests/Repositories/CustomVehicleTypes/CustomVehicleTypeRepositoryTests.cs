...
namespace EPAY.ETC.Core.API.Infrastructure.UnitTests.Repositories.CustomVehicleTypes
{
    public class CustomVehicleTypeRepositoryTests
    {
        #region Init mock data
        private Mock<CoreDbContext> _dbContextMock = new Mock<CoreDbContext>();
        private Mock<ILogger<CustomVehicleTypeRepository>> _loggerMock = new Mock<ILogger<CustomVehicleTypeRepository>>();
        private Mock<DbSet<CustomVehicleTypeModel>>? _dbSetMock;
        #endregion

        #region Init Data mock
        private List<CustomVehicleTypeModel> entities = new List<CustomVehicleTypeModel>
        {
        ...
        public async void GivenRequestIsValidAndExpressionIsExists_WhenGetAllAsyncIsCalled_ThenReturnCorrectResult()
        {
            ...
            Expression<Func<CustomVehicleTypeModel, bool>> expression = s => s.Id == data.Id;
        ...
        public async Task GivenRequestIsValidAndDBContextIsDown_WhenGetAllAsyncIsCalled_ThenThrowETCEPAYCoreAPIException()
        {
            ...
            _dbContextMock.Setup(x => x.CustomVehicleTypes).Throws(someEx);
